...
function extractNameFromTemplate(value) {
  ...
  const lastIndex = value.indexOf('!', 'Hello');
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (indexOfChar === -1) {
      ...
      encodedRot13 += output[indexOfChar];
    }
  }
  return encodedRot13;
}
...
function getCardId(value) {
  ...
  const cardSign = value.slice(-1);

  const valueIndex = cardValues.indexOf(cardValue);
  const signIndex = cardSigns.indexOf(cardSign);

  return signIndex * cardValues.length + valueIndex;
}
